...
const SECTION_LABEL = 'font-display text-[8px] text-accent tracking-[2px] uppercase mb-2';
const PANEL = 'bg-surface border-2 border-border p-4 flex flex-col gap-3 shadow-px';
const BTN_GHOST = 'font-body text-[10px] text-accent border-2 border-accent px-3.5 py-2 bg-transparent cursor-pointer tracking-[0.5px] shadow-px-accent';
const BTN_BLOCK = `${BTN_GHOST} w-full p-3 text-center`;
const BTN_MUTED = 'font-body text-[10px] text-muted border border-border px-3 py-1.5 bg-transparent cursor-pointer';
  ...
  return (
    <div className="px-4 pt-10 pb-20 max-w-lg mx-auto w-full">
      <header className="mb-8">
        <p className="font-body text-[9px] text-muted uppercase tracking-[3px]">App</p>
        <h1 className="font-display mt-2 leading-tight text-[22px] text-accent">SETTINGS</h1>
      </header>
      ...
      <section className="mb-8">
        ...
        <div className={PANEL}>
          {authError && (
            <p className="font-body text-[10px] text-red">✕ Connection failed. Please try again.</p>
          )}
          {connected ? (
            <>
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="font-body text-[9px] text-muted mt-0.5">{email}</p>
                </div>
                <button type="button" onClick={handleDisconnect} className={BTN_MUTED}>
                  DISCONNECT
                </button>
              </div>
              <div className="flex items-center justify-between">
                <div>
                  <p className={`font-body text-[10px] ${STATUS_CLASS[syncStatus]}`}>
      ...
      <section className="mb-8">
        ...
        <div className={PANEL}>
          ...
          <button type="button" onClick={() => fileInputRef.current?.click()} className={BTN_BLOCK}>
            IMPORT 93_HABITS_LOG.MD
          </button>
          <input ref={fileInputRef} type="file" accept=".md,text/markdown" className="hidden" onChange={handleImport} />
          {importError && (
